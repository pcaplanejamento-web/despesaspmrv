...
<text x="${cx}" y="${cy+20}" text-anchor="middle" font-size="9" fill="#D85A30" font-weight="600">${pctMLabel} man.</text>
  ...
  function _buildStep3() {
    const secoes = [
      ['kpis','Indicadores Gerais (KPIs)','Totais, médias e destaques'],
      ['evolucao','Evolução Mensal','Gráfico de barras + tabela mês a mês'],
      ['secretaria','Resumo por Secretaria','Tabela + gráfico de barras'],
      ['classificacao','Por Classificação','Tabela de tipos de serviço'],
      ['ranking','Ranking de Veículos','Top N veículos por gasto'],
      ['detalhe','Detalhamento por Veículo','Despesas individuais de cada veículo'],
      ['historico','Histórico de Secretarias','Trajetória do veículo entre secretarias'],
    ];
...
.capa-footer{font-size:9pt;color:#9ca3af;display:flex;justify-content:space-between;}
/* ─── Seções ─── */
.secao{padding:28px 0;border-bottom:1px solid #f1f5f9;}
.secao:last-child{border-bottom:none;}
.secao-header{display:flex;align-items:center;gap:10px;margin-bottom:18px;padding:10px 14px;background:#f8f9fc;border-left:4px solid #185FA5;border-radius:0 8px 8px 0;}
.secao-header svg{color:#185FA5;flex-shrink:0;}
.secao-header-titulo{font-size:11pt;font-weight:700;color:#1a1f36;letter-spacing:-.1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
/* ─── KPI grid ─── */
...
.kpi-card{background:#f8f9fc;border:1px solid #e5e7eb;border-radius:10px;padding:14px 16px;}
...
<div class="secao avoid-break">
  ...
  <div class="kpi-grid">
    <div class="kpi-card"><div class="kpi-label">Total de despesas</div><div class="kpi-val">${fmtBRL(total)}</div><div class="kpi-sub">${qtde.toLocaleString('pt-BR')} registros</div></div>
    <div class="kpi-card"><div class="kpi-label">Combustível</div><div class="kpi-val" style="color:#185FA5">${fmtBRL(totalC)}</div><div class="kpi-sub">${data.filter(isComb).length} registros</div></div>
    <div class="kpi-card"><div class="kpi-label">Manutenção</div><div class="kpi-val" style="color:#D85A30">${fmtBRL(totalM)}</div><div class="kpi-sub">${data.filter(isManut).length} registros</div></div>
    <div class="kpi-card"><div class="kpi-label">Ticket médio/veículo</div><div class="kpi-val">${fmtBRL(ticketMedio)}</div><div class="kpi-sub">${placasN} veículos ativos</div></div>
    <div class="kpi-card"><div class="kpi-label">Mês de maior gasto</div><div class="kpi-val" style="font-size:12pt">${esc(mesMaior.label||'--')}</div><div class="kpi-sub">${fmtBRL(mesMaior.total||0)}</div></div>
    <div class="kpi-card"><div class="kpi-label">Secretarias ativas</div><div class="kpi-val">${bySigla.length}</div><div class="kpi-sub">${byClassif.length} classificações</div></div>
  </div>
</div>` : ''}
...
<div class="secao">
  ...
  <table>
    <thead><tr><th>Mês / Ano</th><th class="tr">Total</th><th class="tr">Combustível</th><th class="tr">Manutenção</th><th class="tr">Qtde</th><th class="tr">Var.%</th></tr></thead>
    <tbody>
      ${byMes.map((m,i)=>{
        const prev=i>0?byMes[i-1].total:null;
        const varP=prev&&prev>0?((m.total-prev)/prev*100):null;
        const varHtml=varP!==null?`<span style="color:${varP>=0?'#dc2626':'#059669'}">${varP>=0?'+':''}${varP.toFixed(1).replace('.',',')}%</span>`:'—';
        return `<tr><td><strong>${esc(m.label)}</strong></td><td class="tr">${fmtBRL(m.total)}</td><td class="tr" style="color:#185FA5">${fmtBRL(m.comb)}</td><td class="tr" style="color:#D85A30">${fmtBRL(m.manu)}</td><td class="tr small">${m.qtde}</td><td class="tr">${varHtml}</td></tr>`;
      }).join('')}
    </tbody>
    <tfoot><tr class="tr-total"><td><strong>TOTAL</strong></td><td class="tr">${fmtBRL(total)}</td><td class="tr">${fmtBRL(totalC)}</td><td class="tr">${fmtBRL(totalM)}</td><td class="tr small">${qtde}</td><td class="tr">—</td></tr></tfoot>
  </table>
</div>` : ''}
...
<div class="secao">
  <div class="secao-header">
    ...
    <span class="secao-header-titulo">Resumo por Secretaria</span>
  </div>
  <div class="avoid-break" style="margin-bottom:18px">
    ${buildSVGBarH(bySigla.map(s=>({label:s.sigla,total:s.total})),{maxItems:20})}
  </div>
  <table>
    <thead><tr><th>Sigla</th><th>Secretaria / Fundo</th><th class="tr">Total</th><th class="tr">%</th><th class="tr">Combustível</th><th class="tr">Manutenção</th><th class="tr">Veículos</th><th>Proporção</th></tr></thead>
    <tbody>
      ${bySigla.map(s=>{
        const pct=total>0?(s.total/total*100):0;
        const pctComb=s.total>0?(s.comb/s.total*100):0;
        return `<tr>
          <td><strong>${esc(s.sigla)}</strong></td>
          ...
          <td class="tr small">${pct.toFixed(1).replace('.',',')}%</td>
          <td class="tr" style="color:#185FA5">${fmtBRL(s.comb)}</td>
          <td class="tr" style="color:#D85A30">${fmtBRL(s.manu)}</td>
          <td class="tr small">${s.placas}</td>
          <td><div class="bar-cell"><div class="bar-inner" style="width:${Math.max(2,pctComb).toFixed(1)}%;max-width:60px"></div><div class="bar-inner" style="width:${Math.max(2,100-pctComb).toFixed(1)}%;background:#D85A30;max-width:60px"></div></div></td>
        </tr>`;
      }).join('')}
    </tbody>
    <tfoot><tr class="tr-total"><td colspan="2"><strong>TOTAL</strong></td><td class="tr">${fmtBRL(total)}</td><td class="tr small">100%</td><td class="tr">${fmtBRL(totalC)}</td><td class="tr">${fmtBRL(totalM)}</td><td class="tr small">${placasN}</td><td></td></tr></tfoot>
  </table>
</div>` : ''}
...
<div class="secao avoid-break">
  ...
  <table>
    <thead><tr><th>Classificação</th><th class="tr">Qtde</th><th class="tr">Total</th><th class="tr">% do Total</th></tr></thead>
    <tbody>
      ${byClassif.map(c=>`<tr><td>${esc(c.label)}</td><td class="tr small">${c.qtde}</td><td class="tr">${fmtBRL(c.total)}</td><td class="tr small">${fmtPct(c.total,total)}</td></tr>`).join('')}
    </tbody>
    <tfoot><tr class="tr-total"><td><strong>TOTAL</strong></td><td class="tr small">${qtde}</td><td class="tr">${fmtBRL(total)}</td><td class="tr small">100%</td></tr></tfoot>
  </table>
</div>` : ''}
...
${_wiz.secoes.ranking && byPlaca.length ? (() => {
  ...
  <div class="secao-header">
    ...
    <span class="secao-header-titulo">Ranking de Gastos por Veículo${_wiz.topN!=='todos'?' — Top '+_wiz.topN:''}</span>
  </div>
  <table>
    <thead><tr><th style="width:28px">#</th><th>Placa</th><th>Modelo</th><th>Tipo</th><th>Secretaria(s)</th><th class="tr">Total</th><th class="tr">Combustível</th><th class="tr">Manutenção</th><th>Evolução</th></tr></thead>
    <tbody>
      ${topItems.map((v,i)=>{
        const mesVals=_aggByMes(v.entries).map(m=>m.total);
        const isMaq=(v.tipo||'').toLowerCase().startsWith('m');
        return `<tr>
          <td class="small" style="color:#9ca3af">${i+1}</td>
          <td><span style="font-family:monospace;font-weight:700;color:#185FA5">${esc(v.placa)}</span></td>
          <td style="max-width:120px;overflow:hidden;text-overflow:ellipsis">${esc(v.modelo)}</td>
          <td><span class="${isMaq?'badge-m':'badge-v'}">${isMaq?'Máquina':'Veículo'}</span></td>
          ...
          <td class="tr" style="font-weight:700">${fmtBRL(v.total)}</td>
          <td class="tr" style="color:#185FA5">${fmtBRL(v.comb)}</td>
          <td class="tr" style="color:#D85A30">${fmtBRL(v.manu)}</td>
...
<div class="secao">
  <div class="secao-header">
    ...
    <span class="secao-header-titulo">Detalhamento Individual por Veículo</span>
  </div>
  ${byPlaca.map((v,vi)=>{
    const isMaq=(v.tipo||'').toLowerCase().startsWith('m');
    ...
    return `<div class="vei-card avoid-break" style="${vi>0?'margin-top:14px':''}">
      <div class="vei-header">
        ...
        <div class="vei-header-right">
          ...
          <div class="vei-total-sub">${v.qtde} registros</div>
        </div>
      </div>
      <div class="vei-body">
        ...
        ${_wiz.secoes.historico?`<div class="vei-siglas">${Object.values(histSiglas).map(h=>`<span class="vei-sigla-pill" title="${esc(siglaLabel(h.sigla))}">${esc(h.sigla)}</span>`).join('')}</div>`:''}
        <table style="margin-top:4px">
          <thead><tr><th>Período</th><th>Tipo</th><th>Classificação</th><th>Departamento</th>${_wiz.secoes.historico?'<th>Sigla</th>':''}<th class="tr">Valor</th></tr></thead>
          <tbody>
            ${linhas.map(r=>`<tr>
            ...
            ${limitado?`<tr><td colspan="99" style="text-align:center;color:#9ca3af;font-style:italic;padding:6px">... e mais ${(sorted.length-maxLinhas).toLocaleString('pt-BR')} registros (modo executivo)</td></tr>`:''}
          </tbody>
